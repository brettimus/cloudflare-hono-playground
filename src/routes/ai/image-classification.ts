...
/**
 * Get the list of available image classification models
 */
app.get("/models", async (c) => {
  return c.json(aiModelsByType.BaseAiImageClassificationModels);
});

/**
 * Classify an image using a model on Workers AI.
 *
...
app.post("/", async (c) => {
  ...
  const parsedInputs = AiImageClassificationInputSchema.parse(inputs);

  console.log("Using inputs:", parsedInputs);
  const result = await c.env.AI.run(
    model as BaseAiImageClassificationModels,
    parsedInputs,
  );
